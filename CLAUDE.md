...
Sign-In (via Credential Manager, added 2026-08-08) works alongside
email/password — see "Subscriptions / billing" area of this file for
where sign-in and the subscription gate now interact.
...
1. Onboarding + sign-in (email/password + Google) with language picker —
   done, including Google (added 2026-08-08): `AuthRepository.signInWithGoogle`
   takes an ID token from Credential Manager
   (`androidx.credentials` + `googleid`), exchanges it for a Firebase
   credential, and creates the org/member docs on first sign-in exactly
   like `signUp` does — falling back to the Google account's display name
   for the org name, since this flow has no business-name prompt (there's
   no rename UI for that field yet). Required registering **two** SHA-1
   fingerprints in Firebase (Project settings → your app → Add
   fingerprint): the local upload keystore's, and separately the **Play
   App Signing key's** (Play Console → App integrity) — Play re-signs the
   distributed APK with its own key, so only the app-signing SHA-1
   matters for real Play Store installs; both are registered so local/
   internal-track builds also work. `default_web_client_id` (used as
   `setServerClientId(...)`) is auto-generated by the google-services
   Gradle plugin from `google-services.json`'s `oauth_client` array — that
   array is empty until Google is enabled as a sign-in provider in
   Firebase Auth console, so the file has to be re-downloaded after
   enabling it or the app won't compile.
...
9. Settings — language picker (EN/TR, in-app override independent of device
   ...
   user itself. An "Aboneliğim" section shows subscription status/plan/
   renewal date and links to Play's subscription management page — see
